...
async function main() {
  console.log(" Deploying All Contracts to Avalanche Testnet\n");
  ...
  try {
    ...
    console.log(" Deploying AvalancheRacing contract...");
    const AvalancheRacing = await ethers.getContractFactory("AvalancheRacing");

    const racingContract = await AvalancheRacing.deploy();
    await racingContract.waitForDeployment();

    const racingAddress = await racingContract.getAddress();
    console.log("✅ AvalancheRacing deployed to:", racingAddress);

    console.log("\n Deploying AvalancheTournaments contract...");
    const AvalancheTournaments = await ethers.getContractFactory(
      "AvalancheTournaments"
    );

    const tournamentsContract = await AvalancheTournaments.deploy(
      racingAddress
    );
    await tournamentsContract.waitForDeployment();

    const tournamentsAddress = await tournamentsContract.getAddress();
    console.log("✅ AvalancheTournaments deployed to:", tournamentsAddress);
    ...
    console.log(" AvalancheRacing Contract:", racingAddress);
    console.log(" AvalancheTournaments Contract:", tournamentsAddress);
...
main()
  .then(() => {
    console.log(
      "\n ALL CONTRACTS DEPLOYED TO AVALANCHE TESTNET SUCCESSFULLY! "
    );
